...
            // Utilisation de "" au lieu de null est équivalent pour le CSS transition
            h.nextElementSibling.style.maxHeight = "";
        }
    });
...
            if (document.body.classList.contains('accueil')) {
                 // Le setInterval dans startCountdown se gère tout seul après le premier appel
                 startCountdown(); 
            }
            if (document.body.classList.contains('infos')) {
                 initAccordion(); // Nécessaire si les traductions changent le contenu et donc le scrollHeight
            }
        });
    }
