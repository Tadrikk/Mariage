...
const translations = {
  fr: {
    ...
    "infos-whatsapp-button": "Accéder au groupe WhatsApp",
    "infos-whatsapp-prompt": "Entrez le mot de passe pour accéder au groupe WhatsApp :",
    "infos-whatsapp-link": "👉 Rejoindre le groupe WhatsApp",
  ...
  es: {
    ...
    "infos-whatsapp-link": "👉 Unirse al grupo de WhatsApp",
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const guestSet = new Set(guestList); // guestList à définir avant
  ...
  if (rsvpForm && guestNameInput) {
    rsvpForm.addEventListener("submit", async (e) => {
      ...
      // --- Demande du mot de passe RSVP ---
      const pwdPrompt = currentLang === "fr"
        ? "Entrez le mot de passe pour confirmer votre RSVP :"
        : "Introduce la contraseña para confirmar tu RSVP:";
      const password = prompt(pwdPrompt);
      if (!password) return;
      ...
      const correctHash = "88546f608b8f5f0e1451b8da1177b96fec4729f98195755eaf29daba0e597126";

      if (hashHex !== correctHash) {
        alert(currentLang === "fr" ? "Mot de passe incorrect !" : "¡Contraseña incorrecta!");
        return;
      }
  ...
  const whatsappLink = safeGet("whatsappLink");

  if (whatsappButton && whatsappLink) {
    whatsappButton.textContent = translations[currentLang]["infos-whatsapp-button"];

    whatsappButton.addEventListener("click", async () => {
      const pwdPrompt = translations[currentLang]["infos-whatsapp-prompt"];
      const password = prompt(pwdPrompt);
      if (!password) return;
      ...
      if (hashHex === correctHash) {
        ...
        whatsappLink.textContent = translations[currentLang]["infos-whatsapp-link"];
      } else {
...
if (rsvpForm) {
  rsvpForm.addEventListener("submit", async e => {
    ...
    const password = prompt("Entrez le mot de passe pour valider votre RSVP :");
    if (!password) return;
    ...
    if (hashHex === correctHash) {
      thankYou.textContent = translations[currentLang]["thank-you"];
      thankYou.classList.add("show");
      rsvpForm.reset();

      // Optionnel : redirection après confirmation
      setTimeout(() => {
        window.location.href = "index.html";
      }, 2000);
    } else {
